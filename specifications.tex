...
gchar* DecryptageSubstitution( gchar* texteCrypté ,gchar** clef)
	...
	Elle va alors remplacer dans le tableau une lettre du message crypté par la lettre qu'elle semble crypter grace à une 
	conjecture basée sur les fréquences des lettres les plus utilisées et complétée par l'analyse des digrammes et
	trigrammes, elle va egalement créer une clef de substitution sous la forme d'un tableau de caractères de la taille de 
	l'alphabet (la première case correspondra au caractère décryptant la lettre A, la seconde au caractère décryptant la 
	lettre B ect...).
	Cette fonction récupérera la clef de substitution afin de l'enregistrer dans 'clef'.

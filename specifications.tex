...
C'est la plus grande organisation professionnelle technique du monde de l'évolution de la technologie
	...
	ANALYSE AnalyseFrequentielle(gchar* TextClair)\\
		La fonction prend en entrée le text clair à analyser et va ainsi remplir la structure "ANALYSE" et 
...
de figures: soit la fonction ne détécte aucune anomalie est les tableaux en entrée ne sont pas modifiés,
 soit elle détécte des anomalies et les tableau sont alors modifiés avec l'aide de l'utilisateur.
	...
	gchar* DecryptageVigenere(gchar* textCrypte, gchar* safecle);\\
		Cette fonction prend en argument un texte crypté par le chiffrement de Vignère ainsi 
		que safecle (chaine de caractère) qui va permettre de sauvegarder la clé.
		...
		dans un tableau.
		Ensuite appel la fonction Kasiski qui va conjecturé la taille de la clé.
		Ensuite, on déclare un tableau cle[taillekasiski] de taille correspondant à la fonction kasiski. 
	...
	void indiceMutuelle(int cle[],int kasiski, ANALYSE freq, ANALYSE ressource, gchar* safecle);\\
		...
		Elle modifie le tableau cle[] passé en argument qui contiendra l'indice de chacunes des valeurs choisies dans un tableau à 
		une dimension et affecte a safecle la clé afin de pouvoir l'afficher dans le menu. \\

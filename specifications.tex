...
C'est la plus grande organisation professionnelle technique du monde de l'evolution de la technologie
	...
	\section{Signatures et Explications des methodes}
		...
		Le gchar* est une chaîne de caractère, on utilise pas char* car dans la bibliothèque
		GTK+ l'affichage d'un texte ce fait avec gchar*.\\
	...
	struct analyse{ \\
		...
		PHONEME di[];\\
		PHONEME tr[];\\
		gchar* pgor;//plus grande occurence rencontrer\\
	}ANALYSE;\\
	...
	gchar* DecryptageSubstitution( gchar* texte, ANALYSE a)\\
		...
		Aprés avoir appellé la fonction d'analyse fréquentielle, cette fonction vas, 
		dans une boucle, comparer les resultas de l'analyse frequentielle à l'alphabet
		 correspondant à la langue choisit et remplacer une lettre du message crypter 
		 par la lettre qu'elle semble crypter grace à une conjecture basé sur les 
		 fréquences des lettres les plus utilsé et complété par l'analyse des digrammes 
		 et trigrammes. Cette fonction appellera ensuite la fonction VerificationCoerenceDigrammeTrigramme.\\
		Retourne en sortie le texte décrypté (gchar*).\\


	void VerificationCoerenceDigrammeTrigramme(gchar* T[], int *TailleTexte, ANALYSE a)\\
		Prend en argument le texte crypté en cours de décryptage.\\
		Cette fonction vas parcourir le texte en cours de décryptage pour vériffier que les lettres trouvées 
		ne forme pas de digramme ou trigramme inabituelle. Si c'est le cas elle proposera à l'utilisateur 
	...
	\subsubsection{Deryptage Vignere}
	...
	int Kasiski(ANALYSE freq, gchar* TexteCrypte);\\
		Cette fonction cherche les différentes distance de PGOR(Plus Grande Occurence Rencontrée) et effectue
		le PGCD de ces distances. Elle renvoie ce résultat sous forme d'un entier (qui représente la taille du mot clé chercher).\\
	
	void indiceMutuelle(int kasiski, ANALYSE freq, ANALYSE ressource);\\
		Cette fonction calcule les indices de coïncidence mutuelles et les stockes sous forme d'un tableau tab[25][kasiski].\\
		Elle parcourt et compare les 26 valeurs de chaque ligne afin de reperer celle qui sont proche de 0,065 (une par ligne).\\
		Elle renvoie l'indice de chaque des valeurs valides dans un tableau a une dimension. \\
	
	gchar* decrypteur( int T[], int kasiski, gchar* TexteCrypte);\\
		Cette fonction soustrait la valeur du mot clef (tableau renvoyer par indiceMutuelle) au texte Crypté.\\
	...
	gchar* ConvertisseurChar(char T[],int TailleTexte); \\
		Parcoure un tableau donné et le retourne en chaine de caractere\\
	...
	void RecupererChemin(GtkWidget *bouton, GtkWidget *fileselection);\\
		Cette procedure permet de recuperer le chemin d'acces d'un fichier.\\
	...
	void BoiteDialogueVigenere(GtkWidget *Fenetre);\\
		Cette procedure affiche deux zone de texte qui permet de rentrer le texte à crypter et la cle qui permet d'effectuer le chiffrement.\\
		Elle prend en argument une fenetre.\\
		
	void BoiteDialogueAnalyse(GtkWidget *Fenetre);\\
		Cette procedure affiche une zone de texte qui permet de renter le texte sur lequel éffectuer une analyse fréquentielle.\\
		Elle prend en argument une fenetre.\\
